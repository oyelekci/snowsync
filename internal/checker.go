...
func (c *Checker) Check(payload string) (string, error) {
	...
	if itm["external_identifier"] != nil {
		fmt.Printf("Issue id %v has a SNOW identifier: %v", iid, itm["external_identifier"].(string))
		return itm["external_identifier"].(string), nil
	}
